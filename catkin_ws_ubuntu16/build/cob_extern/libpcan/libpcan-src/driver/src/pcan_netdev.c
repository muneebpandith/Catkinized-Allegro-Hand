 ...
 *
 * $Id: pcan_netdev.c 1407 2016-12-30 14:47:45Z stephane $
 *
...
static void pcan_netdev_do_restart(struct work_struct *work)
{
	struct pcandev *pdev = container_of(work, struct pcandev,
						restart_work.work);
...
int pcan_netdev_register(struct pcandev *pdev)
{
	...
	pcan_copy_bt_to_netdev(&priv->can.bittiming, &pdev->init_settings.nominal);
	 ...
	 * interrupt context */
	INIT_DELAYED_WORK(&pdev->restart_work, pcan_netdev_do_restart);
