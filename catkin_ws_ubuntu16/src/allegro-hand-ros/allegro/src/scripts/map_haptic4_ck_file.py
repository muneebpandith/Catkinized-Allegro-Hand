...
class maphaptic:
	def __init__(self):
		...
		self.pubp1 = rospy.Publisher('mx', Floats, queue_size=1)
		self.pubp2 = rospy.Publisher('my', Floats, queue_size=1)
		self.pubp3 = rospy.Publisher('mz', Floats, queue_size=1)
		print("start")
		self.joint_set = []
		urdf_file = file('/home/ubuntu/catkin_ws/src/allegro-hand-ros/allegro_hand_description/allegro_hand_description_right.urdf', 'r')
	...
	def new_one_sided(self):
		...
		while True:
			...
			data2 = rospy.wait_for_message('/glove_out', JointState)
			#flagval = rospy.wait_for_message('/is_grasped',bool)
			print("raw data from hand\n")
			print(data1)

			print("raw data from glove`\n")
			print(data2)

			cur1 = list(data1.position)
			cur2 = list(data2.position)

			print("data from hand\n")
			print(cur1)

			print("data from glove`\n")
			print(cur2)
			...
			print("here is the velocity of finger 1\n")
			print(vel_fing1)
			print("here is the velocity of finger 2\n")
			print(vel_fing2)
			print("here is the velocity of finger 3\n")
			print(vel_fing3)
			print("here is the velocity of finger 4\n")
			print(vel_fing4)
			...
			print("here is the value of objtwistprev 0\n")
			print(objtwistprev[0])
			print("here is the value of objtwistprev 1\n")
			print(objtwistprev[1])
			print("here is the value of objtwistprev 2\n")
			print(objtwistprev[1])
			self.pubp1.publish([objtwistprev[0]])
			self.pubp2.publish([objtwistprev[1]])
			self.pubp3.publish([objtwistprev[2]])
			self.not_basic_map()
			cur2save=cur2
