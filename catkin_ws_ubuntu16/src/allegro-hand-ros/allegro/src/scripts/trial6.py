...
# For estimation of coloumb friction
class my_allegro:
	def __init__(self):
		print("start")
		self.joint_set = []
...
#		self.q1_ik = kdl_kin1.inverse(self.pose1,self.q1_ik)
	...
	def estimator(self):
		...
		with open("test.csv", "a+") as file:
			...
			self.estimator_putit(file,.06,Joint,writer)



	def estimator_putit(self,file,val,Joint,writer):
		for i in range(1000):
			...
			self.qdes2 = np.array([-0.085, 0.57, 0.5045+0*((-1)**(self.count/2)), 1.3555,
								0.084, 0.85, 0.50, 1.10,
								0.1072, 0.06614, -0.0136, 0.0462,
								1.50, 0.43, 0.22, 1.21])
			print(self.qdes2[1])
			for i in range(16):
				Joint.position[i] = self.qdes2[i]
			self.pub.publish(Joint)
		
			self.rate.sleep()
			dt1 = rospy.wait_for_message('/tor',Float32)
			dt2 = rospy.wait_for_message('/vel',Float32)
			writer.writerow([time.time(),dt1.data,dt2.data])
			print(self.count)
